...
\begin{document}
	...
	\label{section:Projection-based reduced basis method}
		...
		This motivates the broad use of \emph{reduced-order} models across several inter-disciplinary areas \cite{HSR16, QMN15}. %, e.g., parameter estimation, optimal control, shape optimization and uncertainty quantification \cite{HSR16, QMN15}. 
	...
	\label{section:Network topology}
		...
		\begin{enumerate}[label=(\roman*)]
			\item MLPs with \emph{one} hidden layer and differentiable activation functions can approximate \emph{any continuous} function; %\cite{Cyb89};
			\label{cybenko-first-rule}
			\item MLPs with \emph{two} hidden layers and differentiable activation functions can approximate \emph{any} function. %\cite{Cyb88}. 
	...
	\label{section:Training a multi-layer feedforward neural network}
		...
		\end{equation*}
		where $\bg{F} : \mathbb{R}^{M_I} \rightarrow \mathbb{R}^{M_O}$ is the actual input-output map established by the neural network and $||\cdot||$ is some suitable norm on $\mathbb{R}^{M_O}$. %To this end, the synaptic and bias weights are iteratively adjusted according to a set of well-defined rules. 
	...
	\label{section:Two-dimensional test case}
		...
		\begin{figure}[t!]		
			...
			\subfloat{\includegraphics[scale = 0.275, trim = {1cm 3cm 4cm 2.5cm}, clip]{poisson2d_3_error_projection}} \\[-0.7cm]
		...
		\begin{figure}[t!]
			\center
			\includegraphics[scale = 0.38, trim = {1.5cm 9.34cm 1.5cm 3.8cm}, clip]{poisson2d_3_cs_error}
			\hspace*{1cm}
	...
	\label{section:The lid-driven cavity problem}
		...
		\end{equation*}
		In our analyses, $\nu(\bg{\mu})$ is adapted as the geometry varies so that the Reynold's number is either $200$ or $400$. \\[0.1cm] %\vspace*{0.2cm} \footnote{We would like to point out that these values for the Reynold's number have been chosen sufficiently low, so that the flow can be reasonably treated as steady.}.
